...
export default defineComponent({
	...
	setup(props) {
		...
		watch(
			...
			(item, prevItem) => {
				...
				presets.value = createPresets(item, {
					...
					realm: leagues.selected.value?.realm || "poe2",
		...
		return {
			...
			makeTradeLink() {
				return `https://${getTradeEndpoint()}/trade2/search/${itemFilters.value.trade.realm}/${itemFilters.value.trade.league}?q=${JSON.stringify(createTradeRequest(itemFilters.value, itemStats.value, props.item))}`;
			},
		};
	},
});
