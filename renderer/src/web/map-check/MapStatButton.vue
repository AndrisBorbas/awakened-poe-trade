<template>
	<div class="flex">
		<button @click="handleClick" :class="[$style.cycleBtn, btnStyle().bg]">
			<i class="text-center w-4 shrink-0" :class="btnStyle().icon" />
			<ItemModifierText
				class="truncate"
				:text="stat.matcher"
				:roll="stat.roll"
...
const entry = computed(() =>
	props.config.selectedStats.find(
		({ matcher }) => matcher === props.stat.matcher,
	),
);
...
function btnStyle() {
	switch (decision.value) {
		case "d":
			return { bg: $style.danger, icon: "fas fa-skull-crossbones" };
		case "w":
			return { bg: $style.warn, icon: "fas fa-exclamation-triangle" };
		case "g":
			return { bg: $style.good, icon: "fas fa-check" };
	}
